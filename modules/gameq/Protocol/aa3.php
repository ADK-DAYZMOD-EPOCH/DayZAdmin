 ...
 *
 * $Id: aa3.php,v 1.3 2009/10/24 18:44:38 evilpie Exp $
 */
require_once GAMEQ_BASE . 'Protocol.php';

/**
 * Americas Army 3 Protocol
 * @author      Richard Perry? <http://www.greycube.com/help/lgsl_other/americas_army_3_query.txt>
 * @author      Tom Schuster <evilpie@users.sf.net>
 * @version     $Revision: 1.3 $
 */
class GameQ_Protocol_aa3 extends GameQ_Protocol {
    ...
    public function status() {
        ...
        while ($this->p->getLength()) {
            ...
            switch ($attribute_type) {
                case 2: #integer
                    $data = $this->p->readInt32();
                    $temp["data"] = $data;
                    break;
                case 7: #array of strings
                    $string_count = $this->p->readInt16();
                    for ($i = 0; $i < $string_count; $i++) {
                        $string = $this->readCustomPascal();
                        if (substr($string, -1) == "\x00") {
                    ...
                    break;
                default:
                    printf("Unused Attribute Type: %d\n", $attribute_type);
            }
            $temp_array[$attribute_name] = $temp;
        }
        for ($i = 0; $i < count($temp_array['attributeNames']['data']); $i++) {
            if (preg_match('/^player(\D+)(\d+)$/', $temp_array['attributeNames']['data'][$i], $matches)) {
                $this->r->addPlayer($matches[1], $temp_array['attributeValues']['data'][$i]);
            } else {
                $key = substr($temp_array['attributeNames']['data'][$i], 6);
                $value = $temp_array['attributeValues']['data'][$i];
                $this->r->add($key, $value);
            }
        }
    }

    public function readCustomPascal() {
        return $this->p->read($this->p->readInt16());
    }

    function ssc_crypt($buffer, $encrypt = FALSE) {
        $master_key = pack("H*", "f5c5914b27235dc0dc274200ddd187c32fe02aed5fc5c079518f49208e4c5548aaef313c5d2e7c91dc580d3cd9e1aec577595325d3c5c84b44a020802becb17e" .
    ...
    public function preprocess($packets) {
        if (count($packets) == 1) {
            ...
            foreach ($packets as $packet) {
                ...
                $sorted_packets[$packet_order] = $packet;
            }
            ksort($sorted_packets);
            $packets = $sorted_packets;
        }

        $data = implode("", $packets);
        $data = $this->ssc_crypt($data, False);
    ...
    public function modifyPacket($packet_conf) {
        $data = $this->ssc_crypt("\x0A\x00playerName\x06\x06\x00query\x00", True);
        $packet_conf['data'] = sprintf($packet_conf['data'], $data);
        return $packet_conf;
    }
